...
\title{Notes on Choice under Uncertainty}
\author{Hsun}
\date{January 8, 2026}
\maketitle
...
\begin{abstract}
    These notes follow MWG in preparation for the Micro Theory II course. They mainly focus on choice under uncertainty. Following Professor Park's syllabus, we focus on Chapters 6, 10 (partial equilibrium), 16, 19, and 13 in this order.
\end{abstract}
...
In a simple lottery, the outcomes that may result are certain. For any compound lottery $(L_1, ..., L_K; \alpha_1, ..., \alpha_K)$, we can calculate a corresponding \textit{reduced lottery} as the simple lottery $\mathrm{L} = (p_1, ..., p_N)$ that generates the same ultimate distribution over outcomes, where $p_n = \alpha_1 p_n^1 + \cdots + \alpha_K p_n^K$. Therefore, $L = \alpha_1 L_1 + \cdots + \alpha_K L_K$.

\subsubsection*{Preference over lotteries}

We assume that \textit{for any risky alternative, only the reduced lottery over final outcomes is relevant to the decision maker.}\footnote{This is the reduction axiom in the notes of investment theory.} Therefore, we let $\mathcal{L}$ be the set of all simple lotteries over the set of outcomes $C$. 

\begin{definition}{6.B.3}
    (\textit{Continuity axiom}) The preference relation $\succcurlyeq$ on the space of simple lotteries $\mathcal{L}$ is \textit{continuous} if for any $L, L', L'' \in \mathcal{L}$, the sets
    \begin{align*}
...
The continuity axiom therefore implies that small changes in probabilities do not change the ordering between two lotteries. Moreover, it excludes the case that the decision maker has lexicographic preferences for alternatives with a zero probability of some outcome. As in Ch3, the continuity axiom implies the existence of a utility function representation.\footnote{See prop. 3.C.1: Suppose the rational $\succcurlyeq$ on $X$ is continuous, then there is a continuous utility function $u(x)$ that represents $\succcurlyeq$.}
...
\begin{definition}{6.B.5}
    (\textit{VNM utility function}) The utility function $U: \mathcal{L} \rightarrow \mathbb{R}$ has a \textit{VNM utility function} form if there is an assignment of numbers $(u_1, \dots, u_N)$ to the $N$ outcomes in $C$ such that for any simple lottery $L = (p_1, \dots, p_N) \in \mathcal{L}$ we have
    \begin{align*}
...
\subsubsection*{Expected utility theorem}
 From the discussion above, we know that if a decision maker's preference over lotteries satisfies the axioms, then it can be represented by a VNM utility function. (The proof is omitted.) We further discuss the properties of indifference curves.\\

 Since the expected utility form is linear in probabilities, representability by the expected utility form is equivalent to the indifference curves being straight and parallel lines. In particular, this is a result of the independence axiom. For example,  
...
\subsection*{6.C Money lotteries and risk aversion}

Let the continuous variable $x$ denote the amount of money, and let the CDF $F: \mathbb{R} \rightarrow [0,1]$ describe the monetary lottery. Also, we take the lottery space $\mathcal{L}$ to be the set of all CDFs over non-negative amounts of money.
...
\end{align*}
The inequality is called \textit{Jensen's inequality}. Therefore, we see that (strict) risk aversion is equivalent to the (strict) concavity of $u(\cdot)$.

\begin{definition}{6.C.2}
    (\textit{Certainty equivalent}) Given $u(\cdot)$, we define 
    \begin{itemize}
        \item The \textit{certainty equivalent} of $F(\cdot)$, $c(F,u)$, is the amount of money for which the agent is indifferent between the gamble $F(\cdot)$ and the certain amount $c(F,u)$. That is,
        \begin{align*}
            u(c(F,u)) = \int u(x) dF(x)
        \end{align*}
        \item For any fixed amount of money $x$ and $\epsilon>0$, the \textit{probability premium}, $\pi(x,\epsilon,u)$, is the excess winning probability over fair odds that makes the individual indifferent between the certain outcome $x$ and a gamble between the two outcomes $x + \epsilon$ and $x - \epsilon$. That is, 
        \begin{align*}
...
\begin{proposition}{6.C.1}
    Suppose a decision maker is an expected utility maximizer with $u(\cdot)$ on amounts of money, then the following are equivalent:
    \begin{itemize}
...
\begin{align*}
    r_A(x) = 4\pi'(0)
\end{align*}
This means that $r_A(x)$ measures \textit{the rate at which the probability premium increases, near certainty, with small risk.}\\

To compare the level of risk aversion across agents, the following criteria seem feasible:
\begin{itemize}
...
\noindent \textbf{Remark:} Note that the "more-risk-averse-than" relation is transitive but \textit{not complete}. That is, we generally have $r_A(x,u_1) > r_A(x,u_2)$ for some $x$ but $r_A(x,u_1) < r_A(x,u_2)$ for some other $x'$.
...
\begin{proposition}{6.C.3}
    ...
    \begin{itemize}
        ...
        \item[(iii)] For any risk $F(z)$, the certainty equivalent formed by adding risk $z$ at wealth level $x$, denoted $c_x$, is such that $(x - c_x)$ is decreasing in $x$. That is, the higher the wealth level $x$ is, the less one is willing to pay to get rid of the risk.
...
Note that the requirement of \textit{non-increasing relative risk aversion} is stronger than that of decreasing absolute risk aversion we studied previously. This is because $r_R(x,u) = x r_A(x,u)$. A risk-averse agent with non-increasing relative risk aversion exhibits decreasing absolute risk aversion, but the converse is not true. The following proposition parallels prop. 6.C.3.
...
In the previous section, we discussed how to compare utility functions. In this section, we compare payoff distributions. Specifically, we construct formal ways to do such a comparison with respect to our two major concerns: return and the dispersion of return. For the following specifications, we restrict ourselves to distributions $F(\cdot)$ such that $F(0) = 0$ and $F(x) = 1$ for some $x$.\\

To see whether the distribution $F(\cdot)$ yields unambiguously higher returns than $G(\cdot)$, we may check two things:
\begin{itemize}
    \item[(i)] Whether every EU-maximizer who prefers more to less prefers $F(\cdot)$ to $G(\cdot)$
    \item[(ii)] For every amount of money $x$, the probability of receiving at least $x$ is higher under $F(\cdot)$ than under $G(\cdot)$
\end{itemize}
Hopefully, it turns out that these two lead to the same concept of \textit{first-order stochastic dominance}. (They are indeed equivalent!)
...
To associate the mean-preserving spread with def. 6.D.2, we need to check two things: 

\begin{itemize}
    \item[(i)] $F(\cdot)$ and $G(\cdot)$ share the same mean
    \item[(ii)] For any non-decreasing concave function $u: \mathbb{R} \rightarrow \mathbb{R}$, the inequality in def. 6.D.2 holds.
\end{itemize}
...
\begin{pf}
    ...
    \begin{align*}
        \int u(x) dG(x) &= \int \left(\int u(x+z) dH_x(z) \right) dF(x) \\
        &\leq \int u\left( \int (x+z) dH_x(z) \right) dF(x) = \int u(x) dF(x)
    \end{align*}
    \hfill $\square$
\end{pf}
...
\begin{definition}{6.D.1**}
    (\textit{Elementary increase in risk}) $G(\cdot)$ forms an \textit{elementary increase in risk} from $F(\cdot)$ if $G(\cdot)$ is generated from $F(\cdot)$ by transferring the mass $F(\cdot)$ assigns to $[x',x'']$ to the endpoints $x'$ and $x''$ such that the mean is preserved.
\end{definition}
...
The figure above illustrates a major difference between state-dependent and \textit{state-uniform} utilities. We call the set of random variables that pay the same amount in every state the \textit{money certainty line}. In a two-state case, as the figure shows, the MRS of a state-dependent utility around the money certainty line is $\frac{\pi_1 u'_1(\bar{x})}{\pi_2 u'_2(\bar{x})}$, which reflects the probability of the occurrence of the state and the nature of state dependence. By contrast, that of a state-uniform utility is only $\frac{\pi_1}{\pi_2}$. (The remaining contents are skipped.)

\subsection*{6.F Subjective probability theory (Optional)}

In many real-world cases, the state of the world is not associated with a recognizable, objective probability but rather is often linked to subjective estimates. The \textit{subjective probability theory}, however, states that there are still restrictions imposed on preferences such that decision makers behave \textit{as if} utilities are assigned to outcomes, probabilities are attached to states of nature, and the decisions are made by taking expected utilities.\\

Following the notation in 6.E, the set of states is denoted by $S = \{1, \dots, S\}$, while the probabilities are not given.
...
Following the concept of Pareto efficiency, we define the \textit{utility possibility set} as $U = \{(u_1, ..., u_I) \in \mathbb{R}^I: \text{there exists a feasible allocation } (x_1, ..., x_I, y_1, ..., y_J) \text{ such that } u_i \leq u_i(x_i) \text{ for all } i\}$.

\subsubsection*{Competitive equilibria}

In this chapter, we are concerned with the analysis of \textit{competitive market economies}. In such an economy, society's initial endowments and technological possibilities (i.e., firms) are owned by consumers. We denote the share of firm $j$ owned by consumer $i$ by $\theta_{ij}$, so that the consumer can claim a fraction $\theta_{ij}$ of firm $j$'s profits. Also, in a competitive economy, a market exists for each good, and all consumers and producers are price takers.\\

\noindent \textbf{Remark:} Recall that in textbooks, we usually argue that the reason why an equilibrium must involve no excess demand or supply is that, for example, some consumers may be better off offering a higher price to receive as many goods as desired. In such a justification, we actually make use of the fact that consumers and producers do not simply take the price as given, which may seem to lead to a contradiction.\\

We have to recognize here that consumers and producers always have the ability to change their offered price. Meanwhile, the price-taking assumption actually refers to the case that there is \textit{no incentive} for them to alter the price: if the (taken-as-given) price equates supply and demand. As long as consumers and producers can make their desired offers at a price that equates supply and demand, consumers will not wish to raise the price and producers will not wish to lower it. This answers the "contradiction" we ran into at first glance.

\begin{definition}{10.B.3}
    (\textit{Competitive equilibrium}) The allocation $(x^*_1, ..., x^*_I, y^*_1, ..., y^*_J)$ and price vector $p^* \in \mathbb{R}^L$ constitute a \textit{competitive equilibrium} (or \textit{Walrasian equilibrium}) if the following conditions are satisfied:
    \begin{itemize}
...
\end{exercise}
\begin{pf}
    \textbf{Proof:} As $y^*_1, ..., y^*_J$ solve the profit maximization problem given $p^*$, they also solve the profit maximization problem under $\alpha p^*$. Plugging this result into consumers' budget constraints, the utility maximization problem becomes:
    \begin{align*}
        \max_{x_i \in X_i} u_i(x_i)\ s.t.\ \alpha p^* \cdot x_i \leq \alpha p^* \cdot \omega_i + \sum_{j=1}^{J} \theta_{ij} (\alpha p^* \cdot y^*_j)
    \end{align*}
...
\begin{pf}
    ...
    \end{align*}
    By clearing markets of goods $l \neq k$, the left hand side equals zero and so does the right hand side ($p_k > 0$). Thus, the market clearing condition holds for the market of good $k$. 
...
Marshallian partial equilibrium analysis envisions the market for one good that constitutes a small fraction of the whole economy. The small size of the market implies two things:
\begin{itemize}
    \item When expenditure on the good is a small portion of a consumer's total expenditure, only a small fraction of any additional dollar of wealth will be spent on it. That is, we expect the wealth effect to be small.
    \item The small size of the market of interest implies that the prices of other goods are approximately unaffected by changes in this market. Therefore, we are justified in treating expenditure on the other goods as a single composite commodity, called the \textit{numeraire}.
\end{itemize}
...
Applying def. 10.B.3 and lemma 10.B.1, we conclude that the allocation $(x^*_1,\dots,x^*_I, q^*_1,\dots,q^*_J)$ and price $p^*$ constitute a competitive equilibrium if and only if 
\begin{align*}
    p^* \leq c'_j(q^*_j),\ &\text{with equality if}\ q^*_j > 0 \quad \forall\ j\\
    \phi'(x^*_i) \leq p^*,\ &\text{with equality if}\ x^*_i > 0 \quad \forall\ i\\
    &\sum_{i=1}^{I} x^*_i = \sum_{j=1}^{J} q^*_j
\end{align*}

The three necessary and sufficient conditions imply that the equilibrium allocation and price are independent of the distribution of endowments and the ownership shares. (This is a consequence of quasi-linear preferences.) In fact, the quasi-linear assumption gives rise to an unlimited unit-for-unit transfer of utility across all consumers through transfers of the numeraire.
...
We may obtain \textit{the optimal consumption and production levels for good $l$} by solving 
\begin{align*}
    \max_{(x_1,...,x_I), (q_1,...,q_J)\geq 0}\ &\sum_{i=1}^{I} \phi_i(x_i) - \sum_{j=1}^{J} c_j(q_j) + \omega_m\\
    &\text{s.t.}\ \sum_{i=1}^{I} x_i - \sum_{j=1}^{J} q_j = 0
\end{align*}

Applying the Lagrangian method (with multiplier $\mu$), we obtain the following conditions:
\begin{align*}
    \mu \leq c'_j(q^*_j),\ &\text{with equality if}\ q^*_j > 0 \quad \forall\ j\\
    \phi'(x^*_i) \leq \mu,\ &\text{with equality if}\ x^*_i > 0 \quad \forall\ i\\
    &\sum_{i=1}^{I} x^*_i = \sum_{j=1}^{J} q^*_j,
\end{align*}
which is exactly def 10.B.3 with $p^*$ replaced by the Lagrange multiplier $\mu$\footnote{This implies that the competitive price is exactly the shadow price on the resource constraint.}. This leads to \textit{the first fundamental theorem of welfare economics}.
...
We can also develop a converse of the first fundamental theorem of welfare economics, called \textit{the second fundamental theorem of welfare economics}. Note that the equilibrium price, consumption, and production levels are unaffected by changes in consumers' wealth levels. Hence, a transfer of one unit of the numeraire from $i$ to $i'$ changes their consumption of the numeraire by exactly the same amount and causes no other change. Hence, by properly transferring the endowment of the numeraire, we may attain any utility vector on the boundary of the utility possibility set.
...
As in Chapter 10, in this chapter, we consider an economy with $I$ consumers, $J$ firms, and $L$ commodities. With our usual notations, such an economy can be characterized by: 
\begin{align*}
...
\begin{notebox}{Recall Ch15}
    In Chapter 15, we are introduced to the useful tool of the \textit{Edgeworth box} in a \textit{pure exchange economy} (i.e., there is no production involved). Here we quickly review some definitions.\\
...
In a private ownership economy, all goods are traded in the market with publicly known prices that consumers and firms take as unaffected by their own actions. Each consumer also has claims to firms' profits. Hence, we characterize such an economy by:
\begin{align*}
...
\begin{exercise}{16.C.1}
    Show that if a consumption set $X_i \subseteq \mathbb{R}^L$ is non-empty, closed, and bounded, and the preference relation $\succcurlyeq_i$ on $X_i$ is continuous, then $\succcurlyeq_i$ cannot be locally nonsatiated.
\end{exercise}
\begin{pf}
...
\subsection*{16.D The second fundamental theorem of welfare economics}

The first fundamental theorem of welfare economics gives no implication about the desirability of the equilibrium allocation from the distributional perspective. Meanwhile, the second fundamental theorem of welfare economics states the partial converse of it, that a Pareto optimal allocation can be supported as a price equilibrium with transfers after a suitable redistribution of initial endowments under some conditions. 

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{figs/fig.15.c.3.png}
\end{figure}

Chapter 15 illustrates many examples of failure of the fundamental theorems of welfare economics when some conditions are not satisfied. To deal with such failures and establish the second fundamental theorem of welfare economics, we introduce the following concept of \textit{price quasi-equilibrium with transfers} which is a weaker version of the price equilibrium with transfers.

\begin{definition}{16.D.1}
    (\textit{Price quasi-equilibrium with transfers}) Given an economy specified by $(\{(X_i, \succcurlyeq_i)\}, \{Y_j\}, \bar{\omega})$, an allocation $(x^*,y^*)$ and a price vector $p \neq \mathbf{0}$ form a \textit{price quasi-equilibrium with transfers} if there is an assignment of wealth $(w_1,...,w_I)$ with $\sum_i w_i = p \cdot \bar{\omega} + \sum_j p \cdot y^*_j$ s.t.
    \begin{itemize}
        \item[(i)] For every firm $j$, $y^*_j$ maximizes profits in $Y_j$. That is,
        \begin{align*}
            p \cdot y_j \leq p \cdot y^*_j\ \text{for all}\ y_j \in Y_j
        \end{align*}
        \item[(ii)] For every consumer $i$, if $x_i \succ_i x^*_i$ then $p \cdot x_i \geq w_i$
        \item[(iii)] Market clearing: $\sum_i x^*_i = \bar{\omega} + \sum_j y^*_j$  
    \end{itemize}
\end{definition}

\noindent \textbf{Remark}: def.16.D.1 weakens def.16.B.4 by replacing the condition that "anything preferred to $x^*_i$ must cost more than $w_i$" (i.e. if $x_i \succ_i x^*_i$ then $p \cdot x_i > w_i$) with "anything preferred to $x^*_i$ can't cost less than $w_i$" (i.e. if $x_i \succ_i x^*_i$ then $p \cdot x_i \geq w_i$). \\

Before introducing the statement of the second fundamental theorem of welfare economics, we recall the separating hyperplane theorem, which is the essence of the proof of the theorem.

\begin{theorem}{}
    (\textit{Separating hyperplane theorem}) Suppose that $B \subseteq \mathbb{R}^N$ is convex and closed and that $x \notin B$. Then, there exists $p \neq \mathbf{0} \in \mathbb{R}^N$ s.t. $p \cdot x > c$ and $p \cdot y < c$ for all $y \in B$.
\end{theorem}

\begin{pf}
    \textbf{Proof}: Consider $x \notin B$ and the point $z \in B$ that is the closest point in $B$ to $x$.\\
    
    Let $p = x-z$ and $c' = p \cdot z$, we see that 
    \begin{align*}
        p \cdot x - c' = p \cdot (x-z) = \|x-z\|^2 > 0\ \Rightarrow p \cdot x > c'
    \end{align*}
    For every point $y \in B$, the vector $p$ and $y-z$ cannot make an acute angle. (otherwise $z$ is not the nearest point to $x$) That is,
    \begin{align*}
        p \cdot (y-z) = p \cdot y - c' \leq 0\ \Rightarrow p \cdot y \leq c'
    \end{align*}
    Let $c = c' + \epsilon$, where $\epsilon > 0$ is taken small enough for $p \cdot x > c' + \epsilon = c > p \cdot y$ as desired.

    \hfill $\square$
\end{pf}

The following is the generalized version of the separating hyperplane theorem.

\begin{theorem}{}
    (\textit{Separating hyperplane theorem}) Consider two non-empty disjoint convex subsets $A,B$ of $\mathbb{R}^N$. Then there exists a non-zero vector $v$ and a real number $c$ such that
    \begin{align*}
        x \cdot v \geq c\ \text{and}\ y \cdot v \leq c\ \text{for all}\ x \in A, y \in B
    \end{align*}
\end{theorem}

\begin{proposition}{16.D.1}
    (\textit{Second fundamental theorem of welfare economics}) Consider an economy specified by $(\{(X_i, \succcurlyeq_i)\}, \{Y_j\}, \bar{\omega})$. Suppose that every $Y_j$ is \textit{convex} and every preference relation $\succcurlyeq_i$ is \textit{convex} (i.e. the set $\{x'_i \in X_i: x'_i \succcurlyeq_i x_i\}$ is convex) and \textit{locally nonsatiated}. Then, for every Pareto optimal allocation $(x^*,y^*)$, there is a price vector $p \neq \mathbf{0}$ s.t. $(x^*,y^*,p)$ is a price quasi-equilibrium with transfers.
\end{proposition}
\begin{pf}
    \textbf{Proof}: For each $i$, define the set $V_i = \{x_i \in X_i: x_i \succ_i x^*_i\} \subseteq \mathbb{R}^L$. Then define the set of aggregate consumption bundles and the aggregate production set as 
    \begin{align*}
        V &= \sum_i V_i = \left\{\sum_i x_i \in \mathbb{R}^L: x_i \in V_i\ \forall\ i \right\}\\
        Y &= \sum_i Y_i = \left\{\sum_i y_i \in \mathbb{R}^L: y_i \in Y_i\ \forall\ i \right\}
    \end{align*}
    Now we prove the following steps:\\

    Step 1: Every $V_i$ is convex.
    \begin{pf}
        \textit{Proof}: Suppose that $x_i, x'_i \in V_i$ and $\alpha \in [0,1]$. By completeness of $\succcurlyeq_i$, we assume WLOG that $x_i \succcurlyeq_i x'_i$. Then, by convexity of $\succcurlyeq_i$, we have
        \begin{align*}
            \alpha x_i + (1-\alpha)x'_i \succcurlyeq_i \alpha x^*_i + (1-\alpha)x^*_i = x^*_i
        \end{align*}
        Note the preference relation is also transitive, we conclude that $\alpha x_i + (1-\alpha)x'_i \in V_i$. Hence, $V_i$ is convex.

        \hfill $\square$
    \end{pf}
    Step 2: $V$ and $Y+\{\bar{\omega}\}$ are convex.
    \begin{pf}
        \textit{Proof}: Simply check that the sum of two convex sets is convex.

        \hfill $\square$
    \end{pf}
\end{pf}
\begin{pf}
    Step 3: $V \cap (Y+\bar{\omega}) = \emptyset$.
    \begin{pf}
        \textit{Proof}: This follows directly from the Pareto optimality of $(x^*_i, y^*_j)$.

        \hfill $\square$
    \end{pf}
    Step 4: There is a non-zero vector $p$ and a real number $r$ s.t. $p \cdot z \geq r$ for all $z \in V$ and $p \cdot z \leq r$ for all $z \in Y+ \{\bar{\omega}\}$.
    \begin{pf}
        \textit{Proof}: This follows from the separating hyperplane theorem.

        \hfill $\square$  
    \end{pf}
    Step 5: If $x_i \succcurlyeq_i x^*_i$ for all $i$, then $p \cdot (\sum_i x_i) \geq r$.
    \begin{pf}
        \textit{Proof}: By local nonsatiation, for each $i$ there is a consumption bundle $x'_i$ that is arbitrarily close to $x_i$ s.t. $x'_i \succ_i x_i$. Hence, $\sum_i x'_i \in V$ and $p \cdot \sum_i x'_i \geq r$.\\
        
        Taking the limit, we have $p \cdot \sum_i x_i \geq r$.

        \hfill $\square$
    \end{pf}
    Step 6: $p \cdot (\sum_i x^*_i) = p \cdot (\bar{\omega} + \sum_j y^*_j) = r$
    \begin{pf}
        \textit{Proof}: By step 5, we have $p \cdot (\sum_i x^*_i) \geq r$.\\
        
        On the other hand, note $\sum_i x^*_i = \sum_j y^*_j + \bar{\omega} \in Y + \{\bar{\omega}\}$. So we have by step 4 that $p \cdot (\sum_i x^*_i) \leq r$.

        \hfill $\square$
    \end{pf}
    Step 7: For every $j$, we have $p \cdot y_j \leq p \cdot y^*_j$ for all $y_j \in Y_j$. (condition (i))
    \begin{pf}
        \textit{Proof}: Note that for all firm $j$ and all $y_j \in Y_j$, we have $y_j + \sum_{k \neq j}y^*_k \in Y$. Therefore, by step 4 and 6
        \begin{align*}
            p \cdot \left(\bar{\omega} + y_j + \sum_{k \neq j}y^*_k\right) \leq r = p \cdot \left(\bar{\omega} + \sum_j y^*_j \right)
        \end{align*}
        Hence, $p \cdot y_j \leq p \cdot y^*_j$.

        \hfill $\square$
    \end{pf}
\end{pf}
\begin{pf}
    Step 8: For every $i$, if $x_i \succ_i x^*_i$, then $p \cdot x_i \geq p \cdot x^*_i$. (condition (ii))
    \begin{pf}
        \textit{Proof}: Consider any $x_i \succ_i x^*_i$, we have by step 5 and 6 that
        \begin{align*}
            p \cdot \left(x_i + \sum_{k \neq i}x^*_k \right) \leq p \cdot \left(\sum_i x^*_i \right)
        \end{align*}
        Hence, $p \cdot x_i \geq p \cdot x^*_i$.

        \hfill $\square$
    \end{pf}
    Step 9: The wealth levels $w_i = p \cdot x^*_i$ for all $i$ support $(x^*,y^*,p)$ as a price quasi-equilibrium with transfers.
    \begin{pf}
        \textit{Proof}: We are left to check that condition (iii) holds. And this follows from the feasibility of the Pareto optimal allocation $(x^*, y^*)$.

        \hfill $\square$
    \end{pf}
\end{pf}

The figure below illustrates what we've done in the proof.

\begin{figure}[H]
\centering
\includegraphics[width=0.5\textwidth]{figs/fig.16.d.1.png}
\end{figure}

The next problem we would like to address is that under what conditions, a quasi-equilibrium can be supported as an equilibrium. To this end, we need to explore the relation between the condition (ii) in these two different definitions.

\begin{proposition}{16.D.2}
    Let $X_i$ be convex and $\succcurlyeq_i$ be continuous.\\
    
    Suppose that $x^*_i \in X_i, y^*_j \in Y_j, p$, and $w_i$ are s.t.
    \begin{align*}
        x_i \succ_i x^*_i\ \Rightarrow\ p \cdot x_i \geq w_i
    \end{align*}
    Then, if there is a consumption vector $x'_i \in X_i$ s.t. $p \cdot x'_i < w_i$, then
    \begin{align*}
        x_i \succ_i x^*_i\ \Rightarrow\ p \cdot x_i > w_i
    \end{align*}
\end{proposition}
\begin{pf}
    \textbf{Proof}: The proof is depicted in the figure below.
    
    \begin{figure}[H]
    \centering
    \includegraphics[width=0.4\textwidth]{figs/fig.16.d.3.png}
    \end{figure}

    Suppose by contradiction, there exists $x_i \succ_i x^*_i$ with $p \cdot x_i = w_i$.\\
    
    Since there exists $x'_i \in X_i$ s.t. $p \cdot x'_i < w_i$, we have $\alpha x_i + (1-\alpha)x'_i \in X_i$ and $p \cdot (\alpha x_i + (1-\alpha)x'_i) < w_i$ for all $\alpha \in [0,1)$. \\

    Taking $\alpha$ close enough to 1, the continuity of $\succcurlyeq_i$ implies that $\alpha x_i + (1-\alpha)x'_i \succ_i x^*_i$, which is a contradiction as the bundle is strictly better than $x^*_i$ while costs less than $w_i$.

    \hfill $\square$
\end{pf}

\begin{proposition}{16.D.3}
    Suppose that $X_i$ is convex for all $i$, $\mathbf{0} \in X_i$, and $\succcurlyeq_i$ is continuous. Then any price quasi-equilibrium with transfers that has $(w_1, ...w_I) \gg 0$ is a price equilibrium with transfers.
\end{proposition}
\begin{pf}
    \textbf{Proof}: This is a direct consequence of prop 16.D.2. 

    \hfill $\square$
\end{pf}

\subsection*{16.E Pareto optimality and social welfare optima}

\begin{definition}{}
    Given the continuous utlity functions $u_i(\cdot)$, we capture the attainable vectors of utility levels by the \textit{utility possibility set}:
\begin{align*}
    U = \{(u_1,...u_I) \in \mathbb{R}^I:\ &\text{there is a feasible allocation}\ (x,y)\ \text{s.t.}\ \\
      &u_i \leq u_i(x_i)\ \text{for}\ i = 1,...,I \}
\end{align*}

Also, we define the \textit{Pareto frontier} by
\begin{align*}
    UP = \{(u_1,...u_I) \in U :\ &\text{there is no}\ (u'_1,...,u'_I) \in U\ \text{s.t.}\ u'_i \geq u_i\\
    &\text{for all}\ i\ \text{and}\ u'_i > u_i\ \text{for some}\ i \}
\end{align*}
\end{definition}

\begin{proposition}{16.E.1}
    A feasible allocation $(x,y) = (x_1,...,x_I,y_1,...,y_J)$ is a Pareto optimum \textit{if and only if} $(u_1(x_1),...,u_I(x_I)) \in UP$. 
\end{proposition}
\begin{pf}
    \textbf{Proof}: $(\Rightarrow)$ Suppose $(u_1(x_1),...,u_I(x_I)) \notin UP$, then there exists $(u'_1,...,u'_I) \in U$ s.t. $u'_i \geq u_i$ for all $i$ and $u'_i > u_i$ for some $i$. This shows that $(x',y')$ Pareto dominates $(x,y)$, which is a contradiction.

    \hfill $\square$

    $(\Leftarrow)$ Suppose by contradiction that $(x,y)$ is not Pareto optimal, then there exists some feasible allocation $(x',y')$ that Pareto dominates $(x,y)$. That is, $u_i(x'_i) \geq u_i(x_i)$ for all $i$ and $u_i(x'_i) > u_i(x_i)$ for some $i$. Hence, $(x,y) \notin UP$, which is a contradiction.

    \hfill $\square$
\end{pf}

\begin{definition}{}
    (\textit{Social welfare function}) We describe the society's distributional principle in a \textit{social welfare function} $W(u_1,...,u_I)$. In particular, we take the linear form s.t $W(u_1,...,u_I) = \sum_i \lambda_i u_i = \lambda \cdot u,\ \lambda \geq 0$.
\end{definition}

\begin{proposition}{16.E.2}
    If $u^*$ is a solution to the social welfare maximization problem with $\lambda \gg 0$, then $u^* \in UP$. Moreover, if the utility possibility set $U$ is convex, then for any $\tilde{u} \in UP$, there exists a vector $\lambda \geq 0,\ \lambda \neq \mathbf{0}$ s.t. $\lambda \cdot \tilde{u} \geq \lambda \cdot u$ for all $u \in U$.
\end{proposition}
\begin{pf}
    \textbf{Proof}: (i) Suppose $u^*$ is not Pareto optimal, then there exists $u \in U$ with $u \geq u^*$ and $u \neq u^*$. Combining with the fact that $\lambda \gg 0$, we have $\lambda \cdot u > \lambda \cdot u^*$.

    \hfill $\square$

    (ii) Suppose $\tilde{u} \in UP$, then $\tilde{u}$ is in the boundary of $U$. By the separating hyperplane theorem, there exists a vector $\lambda \neq \mathbf{0}$ s.t. $\lambda \cdot \tilde{u} \geq \lambda \cdot u$ for all $u \in U$. Moreover, $\lambda \geq 0$ as otherwise by choosing $u \in U$ with $u_i < 0$ large enough in absolute value, we would have $\lambda \cdot u > \lambda \cdot \tilde{u}$.

    \hfill $\square$
\end{pf}

\noindent \textbf{Remark}: Prop.16.E.2 shows that for economies with convex utility possibility sets, every linear social welfare optimum with weights $\lambda \gg 0$ is Pareto optimal, and every Pareto optimal allocation can be supported as a social welfare optimum with some $\lambda \geq 0$.


\newpage

\section*{Ch19: General equilibrium under uncertainty}
\hfill

\subsection*{19.B A market economy with contingent commodities: description}

In this chapter, we study economies where technologies, endowments, and preferences are uncertain. In particular, they are assumed to depend on the state of the world. We also assume that the set of states of the world, $S$, is given and finite.

\begin{definition}{19.B.1}
    (\textit{(State-)contingent commodity}) For every physical commodity $l = 1,...,L$ and state $s = 1, ..., S$, a unit of (state-)contingent commodity $ls$ is a title to receive a unit of the physical good $l$ \textit{if and only if} $s$ occurs. Accordingly, a \textit{(state-)contingent commodity vector} is specified by 
    \begin{align*}
        x = (x_{11}, \dots, x_{L1}, \dots, x_{1S}, \dots, x_{LS}) \in \mathbb{R}^{LS}
    \end{align*}
    and is an entitlement to receive the commodity vector $(x_{1s},...,x_{Ls})$ if $s$ occurs.
\end{definition}

\subsection*{19.C Arrow-Debreu equilibrium}

Following def.19.B.1, we describe the economy under uncertainty by means of a set of states of the world $S$, a consumption set $X_i \subseteq \mathbb{R}^{LS}$, an endowment vector $\omega_i \in \mathbb{R}^{LS}$, and a preference relation $\succcurlyeq_i$ on $X_i$ for all consumers $i$, together with a production set $Y_j \subseteq \mathbb{R}^{LS}$ and profit shares $(\theta_{j1}, ..., \theta_{jI})$ for every firm $j$.\\

In the economy we study, what is purchased (or sold) for the contingent commodity $ls$ is a commitment to receive (or deliver) amounts of the good $l$ if $s$ occurs. Hence, we introduce a new equilibrium concept:

\begin{definition}{19.C.1}
    (\textit{Arrow-Debreu equilibrium}) An allocation
    \begin{align*}
        (x^*_1,...,x^*_I,y^*_1,...,y^*_J) \in X_1 \times ... \times X_I \times Y_1 \times Y_J \subseteq \mathbb{R}^{LS(I+J)}
    \end{align*}
    and a system of prices for the contingent commodities $p = (p_{11},...,p_{LS}) \in \mathbb{R}^{LS}$ form an \textit{Arrow-Debreu equilibrium} if 
\end{definition}
\begin{notebox}{}
    \begin{itemize}
        \item[(i)] For every $j$, $y^*_j$ satisfies $p \cdot y^*_j \geq p \cdot y_j$ for all $y_j \in Y_j$
        \item[(ii)] For every $i$, $x^*_i$ is maximal for $\succcurlyeq_i$ in the budget set
        \begin{align*}
            \{x_i \in X_i: p \cdot x_i \leq p \cdot \omega_i + \sum_{j} \theta_{ij} p \cdot y^*_j \}
        \end{align*}
        \item[(iii)] $\sum_i x^*_i = \sum_j y^*_j + \sum_i \omega_i$
    \end{itemize}
\end{notebox}
