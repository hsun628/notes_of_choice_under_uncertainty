...
The following is the genralized version of the separating hyperplane theorem.

\begin{theorem}{}
    (\textit{Separating hyperplane theorem}) Consider two non-empty disjoint convex subsets $A,B$ of $\mathbb{R}^N$. Then there exists a non-zero vector $v$ and a real number $c$ s.t.
    \begin{align*}
...
\begin{pf}
    ...
    \begin{pf}
        ...
        On the other hand, note $\sum_i x^*_i = \sum_j y^*_j + \bar{\omega} \in Y + \{\bar{\omega}\}$. So we have by step 4 that $\sum_i x^*_i \leq r$.
...
\begin{pf}
    ...
    \begin{pf}
        ...
        \hfill $\square$
    \end{pf}
    Step 9: Thhe wealth levels $w_i = p \cdot x^*_i$ for all $i$ support $(x^*,y^*,p)$ as a price quasi-equilibrium with transfers.
    \begin{pf}
...
\noindent \textbf{Remark}: Prop.16.E.2 shows that for ecnomies with convex utility possibility sets, every linear social welfaer optimum with weights $\lambda \gg 0$ is Pareto optimal, and every Pareto optimal allocation can be supported as a social welfare optimum with some $\lambda \geq 0$.
...
\subsection*{A market economy with contingent commodities: description}
